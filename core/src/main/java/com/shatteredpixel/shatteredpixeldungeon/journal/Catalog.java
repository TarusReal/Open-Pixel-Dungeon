...
public enum Catalog {
	...
	static {
		...
		//items with no Generator category at all live in ItemRegistry (audit finding #1) - see
		//that class's comment. Buckets that DO have a Generator category (POTIONS, SCROLLS, ...)
		//keep deriving straight from Generator.Category.X.classes below, unchanged.
		MISC_EQUIPMENT.addItems(ItemRegistry.MISC_EQUIPMENT);
		...
		FOOD.addItems(ItemRegistry.FOOD_EXTRA);
		...
		BOMBS.addItems(ItemRegistry.BOMBS);

		TIPPED_DARTS.addItems(TippedDart.types.values().toArray(new Class[0]));

		BREWS_ELIXIRS.addItems(ItemRegistry.BREWS_ELIXIRS);

		SPELLS.addItems(ItemRegistry.SPELLS);

		MISC_CONSUMABLES.addItems(Generator.Category.GOLD.classes);
		MISC_CONSUMABLES.addItems(ItemRegistry.MISC_CONSUMABLES);
